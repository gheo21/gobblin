 ...
 * The {@link AvroToParquetHdfsTimePartitionedWriter#write(GenericRecord)} takes each record, extracts out the the value of the
 ...
 */
public class AvroToParquetHdfsTimePartitionedWriter implements DataWriter<GenericRecord> {
  private static final Logger LOG = LoggerFactory.getLogger(AvroToParquetHdfsTimePartitionedWriter.class);
   ...
   */
  protected final Map<Path, AvroToParquetHdfsDataWriter> pathToWriterMap = Maps.newHashMap();
  ...
  protected void write(GenericRecord record, long recordTimestamp) throws IOException {
    ...
    if (!this.pathToWriterMap.containsKey(writerOutputPath)) {
      ...
      AvroToParquetHdfsDataWriter parquetHdfsDataWriter = createParquetHdfsDataWriterForPath(writerOutputPath);

      this.pathToWriterMap.put(writerOutputPath, parquetHdfsDataWriter);
    }
  ...
  public void commit() throws IOException {
    ...
    for (Entry<Path, AvroToParquetHdfsDataWriter> entry : this.pathToWriterMap.entrySet()) {
      try {
   ...
   * {@link DataWriter} will have all the same properties use to construct {@link AvroToParquetHdfsTimePartitionedWriter}, except
   ...
   */
  private AvroToParquetHdfsDataWriter createParquetHdfsDataWriterForPath(Path path) throws IOException {
    ...
    return (AvroToParquetHdfsDataWriter) new AvroToParquetDataWriterBuilder()
