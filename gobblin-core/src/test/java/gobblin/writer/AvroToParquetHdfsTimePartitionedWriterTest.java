...
public class AvroToParquetHdfsTimePartitionedWriterTest {
  ...
  private static final String SIMPLE_CLASS_NAME = AvroToParquetHdfsTimePartitionedWriterTest.class.getSimpleName();
  ...
  public static void setUp() throws IOException {
    ...
    writer =
        new AvroToParquetTimePartitionedWriterBuilder().writeTo(Destination.of(Destination.DestinationType.HDFS, properties))
            .writeInFormat(WriterOutputFormat.PARQUET).withWriterId(WRITER_ID).withSchema(schema).withBranches(1)
            .forBranch(0).build();
  }
  ...
  public void testWriter() throws IOException {
    ...
    Assert.assertEquals(FileUtils.listFiles(new File(TEST_ROOT_DIR), new String[] { "parquet" }, true).size(), 3);
